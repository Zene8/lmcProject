...
public class LMCFormatter {
    ...
    // FIX: Pattern now correctly includes DAT and is consistent with the parser.
    private static final Pattern LINE_PATTERN = Pattern.compile(
            "^\\s*" +
                    "(?:([A-Za-z_][A-Za-z0-9_]*):\\s*)?" + // Optional Label (Group 1)
                    "(INP|OUT|LDA|STA|ADD|SUB|BRA|BRZ|BRP|HLT|DAT)" + // Instruction (Group 2)
                    "(?:\\s+([A-Za-z_][A-Za-z0-9_]*|\\d+))?" + // Optional Operand (Group 3)
                    "(\\s*//.*)?" + // Optional Comment (Group 4)
                    "\\s*$",
